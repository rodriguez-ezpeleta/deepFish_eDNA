...
names=(`cut -f 1 Teleo_samples_20200317.txt`)
R1=(`cut -f 2 Teleo_samples_20200317.txt`)
R2=(`cut -f 3 Teleo_samples_20200317.txt`)

echo -n "" > fasta_file.txt
echo -n "" > group.txt
echo -n "" > Teleo_samples_20200317.fasta
echo "Sample	raw	cut	assembled	retained" > reads_stats.txt

for i in `seq 0 172`; do
...
#convert into fasta 
	...
	if (($i > 171)) ; then # number must be = i-2
	...
	cat ${names[i]}.assembled_retained.fasta >> Teleo_samples_20200317.fasta
...
mv mergegroups Teleo_samples_20200317.groups
rm -f mothur*logfile
